programa
{
	...
	funcao desenhar_ondas(inteiro largura_janela, inteiro altura_janela, inteiro espacamento)
	{
		...
		para(inteiro i = 0; i<= largura_janela/espacamento; i++)
		{
			...
			{
				g.desenhar_retangulo(i*espacamento+ espacamento*m.cosseno((fase + i*espacamento)*2*m.PI/360),j*espacamento+espacamento*m.seno((fase+j*espacamento)*2*m.PI/360), 4, 4, falso, verdadeiro)
			}
		}
		g.renderizar()
		u.aguarde(5)
